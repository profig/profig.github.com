...
$(document).ready(function(){
		...
		// $(window).on("load",function(){
			...
			// loadPage('phonesystem');	//Change default loading page here
		// });
		...
		$('.frontLayer .menuButton').on("click",function(){
...
function dataLinks(dom) {
	//This is what happens when you click a button on sidebar
	$("[data-link]", dom).on("click",function(e){
		e.preventDefault();
...
function loadPage(page){
	$.ajax({
		url:"/account/"+page.toString()+".html",
		cache:false,
		success: function(data){
			$(".content").empty();
			$(".content").html(data);
